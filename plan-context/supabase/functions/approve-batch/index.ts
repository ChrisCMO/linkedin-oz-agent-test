...
/** Hash the raw token to compare against token_hash in DB */
...
/** Trigger Oz cloud agent to send invites for this batch */
...
/** Build the HTML confirmation page shown after approval */
...
serve(async (req: Request) => {
  ...
  // 5. Update all prospects in batch to 'approved' status
  ...
  if (prospectIds.length > 0) {
    await sb
      ...
      .in("id", prospectIds)
      .in("status", ["scored", "queued"]); // Only update if in reviewable state
  }
  ...
  // 7. Trigger Oz agent (fire-and-forget — don't block the response)
  const batchInfo = `${prospectIds.length} prospects for campaign ${batch.campaign_id}.`;
  triggerOzAgent(batchId, batchInfo); // intentionally not awaited
